...
const
  ...
  RowNames: array[Rows, string] = ["Sponza", "Flight helmet", "Character", "Damaged helmet",
...
proc frame(dt, tickFraction: float) =
  ...
  # the 3D model
  g.character.setTint(if g.scene.getHover(g.character) in {ButtonState.Pressed, ButtonState.Down}:
                      g.highlight else: ColorWhite)
  if g.scene.isClicked(g.character):
  ...
  let hoveredName =
    ...
    elif hovered == g.character: "the character"

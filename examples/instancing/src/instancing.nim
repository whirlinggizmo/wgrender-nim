...
## differs only in transform and tint, so it is one draw; six characters share the same
...
proc loadMeshInto(model: Model) =
  ...
  if not ensureAssetAsync(CharacterPath).addTask(onReady, onFailed):
    onFailed(CharacterPath)
